...
{
    eepromMetadata = MetaData<eepromValues_t>::GenerateNewMetaData(eepromValuesNew);
    ...
    if (validateWriteData.versionTimestampXOR != eepromMetadata.versionTimestampXOR)
    {
        LOG_ERROR("Write/Read on EEPROM Failed: Written XOR: %d, Read XOR: %d", eepromMetadata.versionTimestampXOR, validateWriteData.versionTimestampXOR);
        return false;
    }
    return true;
}
...
void eepromInit()
{
    eepromMetadata = MetaData<eepromValues_t>::GenerateNewMetaData(getEepromDefaults());
    ...
    if (version == CurrentEEPROMVersion)
    {
        ...
        {
            LOG_ERROR("Timestamp and Version XOR did not equal the expected output, adding fresh values to eeprom");
            LOG_DEBUG("Calculated XOR: %d, Expected XOR: %d", xorOutput, eepromMetadata.versionTimestampXOR);
    ...
    {
        LOG_INFO("EEPROM did not find a valid version in the EEPROM, writing new data");
        if (!eepromWrite(getEepromDefaults()))
        {
            LOG_ERROR("Failed to write to EEPROM");
        }
